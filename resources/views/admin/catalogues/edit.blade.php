<x-app-layout>
    <div class="max-w-7xl mx-auto shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold text-[#401457]">Edit Catalogue</h2>
        <form action="{{ route('catalogues-backend.update', $catalogues->id) }}" method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="name_en" class="block text-sm font-medium text-[#000]">Title (English)</label>
                    <input value="{{ old('name_en', $catalogues->name_en) }}" type="text" name="name_en"
                        id="name_en" class="mt-1 block w-full p-2 border rounded-md text-[#000] text-sm">
                    <x-input-error class="mt-2" :messages="$errors->get('name_en')" />
                </div>

                <div>
                    <label for="name_km" class="block text-sm font-medium text-[#000]">Title (Khmer)</label>
                    <input value="{{ old('name_km', $catalogues->name_km) }}" type="text" name="name_km"
                        id="name_km" class="mt-1 block w-full p-2 border rounded-md text-[#000] text-sm">
            ...
            <div>
                ...
                <label for="dropzone-file{{ $catalogues->id }}" id="drop-area"
                    ...
                    <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                        id="img-preview" style="background-image: url('{{ asset($catalogues->image) }}')">
                    ...
                    <input id="dropzone-file{{ $catalogues->id }}" type="file" class="hidden" name="image"
