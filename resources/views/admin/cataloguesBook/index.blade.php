...
@section('content')
    <div class="">
        <div class="my-4 flex items-center gap-4 justify-end">
            <a href="{{ route('catabook-backend.create') }}"
            ...
            <form method="GET" action="{{ route('catabook-backend.index') }}" id="filterForm" class="">
                <div class="flex items-center gap-3">
                    <label for="catalogue_id" class="font-semibold">Catalogue:</label>
                    <select name="catalogue_id" id="catalogue_id"
                        class="border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-blue-400"
                    ...
                    @foreach ($cata_books as $index => $cata_book)
                        <tr class="border border-[#4FC9EE]">
                            ...
                            <td class="text-left py-3 px-4 text-[10px] md:text-[12px] border-r border-[#4FC9EE]">
                                <img src="{{ asset($cata_book->default_image) }}" alt=""
                                    class="w-12 h-10 object-contain">
                            </td>
                            <td
                                class="text-left py-3 px-4 text-[10px] md:text-[12px] max-w-[100px] truncate border-r border-[#4FC9EE]">
                                {{ $cata_book->name_en }}</td>
                            <td
                                class="text-left py-3 px-4 text-[10px] md:text-[12px] max-w-[100px] truncate border-r border-[#4FC9EE]">
                                {{ $cata_book->type_en }}</td>
                            <td
                                class="text-left py-3 px-4 text-[10px] md:text-[12px] max-w-[100px] truncate border-r border-[#4FC9EE]">
                                {{ $cata_book->size_en }}</td>
                            <td
                                class="text-left py-3 px-4 text-[10px] md:text-[12px] max-w-[100px] truncate border-r border-[#4FC9EE]">
                                {{ $cata_book->code }}</td>
                            <td
                            ...
                            <td class="text-left py-3 px-4">
                                <div class="flex">
                                    ...
                                    <a href="{{ route('catabook-backend.edit', ['catabook' => $cata_book->id, 'page' => request('page')]) }}"
                                        title="Edit">
        ...
        <div class="pt-2 pb-4">
            {{-- {{ $cata_books->links() }} --}}
            {{ $cata_books->appends(request()->query())->links() }}
