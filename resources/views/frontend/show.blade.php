...
@section('content')
    ...
    <div class="max-w-7xl mx-auto py-10 px-4 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div
            class="w-full lg:col-span-2 relative pl-4 lg:pl-10 mt-4 md:mt-0 lg:before:content-[''] lg:before:absolute lg:before:right-0 before:top-0 lg:before:h-full lg:before:w-[2px] lg:before:bg-[#4FC9EE]">
            <div>
            ...
            <h1 class="text-[14px] md:text-[15px] font-[700]">
            ...
            <a href="{{ route('news_item') }}"
                class="inline-block mt-6 px-4 py-2 bg-gray-800 text-white rounded hover:bg-[#4FC9EE] transition-all duration-300 hover:tracking-[0.5px]">
                ← {{ $locale === 'km' ? 'ព័ត៌មានបន្ថែម' : 'Back' }}
            </a>
        </div>
        ...
        <div class="flex flex-wrap flex-row lg:flex-col gap-4" x-data="{ showAll: false }">
            @foreach ($newsItem as $index => $item)
                ...
                <div x-show="showAll || {{ $index }} < 2" x-transition
                    class="relative w-[48%] lg:w-full h-[200px] group overflow-hidden">
