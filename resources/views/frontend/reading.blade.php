...
shareText() { const pageUrl = window.location.href; const shareContent = `${this.selectedText}\n\n${this.numberChapter}`; if (navigator.share) { navigator.share({ title: document.title, text: shareContent, url: pageUrl }).catch(() => {}); } else { navigator.clipboard.writeText(pageUrl) .then(() => { this.copied = true; setTimeout(() => this.copied = false, 1500); }); } },
    closePopup() {
